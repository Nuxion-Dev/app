...
export const defaultCrosshairs: CrosshairItem[] = [
  ...
  },
  {
    ...
    content: ({ className, style }) => (
      ...
      >
        <g>
          <path d="M10 10 H50 V90 H90 M10 90 V50 H90 V10" fill="none" stroke="currentColor" strokeWidth="5" />
        </g>
      </svg>
    ),
  },
  {
    id: "svg11",
    content: ({ className, style }) => (
      <svg width="41" height="41" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" className={className} style={style} preserveAspectRatio="xMidYMid meet" shapeRendering="crispEdges">
        <g fill="currentColor">
          <rect x="19" y="6" width="2" height="6"/>
          <rect x="19" y="28" width="2" height="6"/>
          <rect x="6" y="19" width="6" height="2"/>
          <rect x="28" y="19" width="6" height="2"/>
          <rect x="19" y="6" width="2" height="6" transform="rotate(45 20 20)"/>
          <rect x="19" y="6" width="2" height="6" transform="rotate(90 20 20)"/>
          <rect x="19" y="6" width="2" height="6" transform="rotate(135 20 20)"/>
          <rect x="19" y="6" width="2" height="6" transform="rotate(315 20 20)"/>
        </g>
      </svg>
    ),
  },
];
export const crosshairById: Record<string, CrosshairItem> = Object.fromEntries(
  defaultCrosshairs.map((c) => [c.id, c])
) as Record<string, CrosshairItem>;

export const getCrosshairById = (id: string | null | undefined): CrosshairItem => {
  if (!id) return defaultCrosshairs[0];
  return crosshairById[id] ?? defaultCrosshairs[0];
};
...
export const renderCustomCrosshair = (grid: boolean[][], props: CrosshairProps) => {
  ...
  return (
    <svg viewBox={`0 0 ${totalSize} ${totalSize}`} className={props.className} style={props.style}>
      {grid.map((row, r) =>
        row.map((cell, c) =>
          cell ? (
            <rect
