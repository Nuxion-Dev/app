...
export default function Games() {
    ...
    const filteredGames = useMemo(() => {
        ...
        switch (sort) {
            ...
            case "name-desc":
                ...
                break;
            case "favourites":
                updatedGames = updatedGames.sort((a, b) => (b.favourite ? 1 : 0) - (a.favourite ? 1 : 0));
                break;
            case "last-played":
    ...
    const onAdd = async ({ name, exe, args, banner }: CustomGameInfo) => {
        try {
            const game = await addCustomGame(name, exe, args, banner);
            if (game) setGames((prev) => [...prev, game]);
        } catch (error) {
            console.error("Failed to add custom game:", error);
            setError("Failed to add custom game.");
        }
    }
