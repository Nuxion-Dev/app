...
impl RenderEngine for D3D11RenderEngine {
    ...
    fn render(
        ...
        // Fast path: no imgui draw commands were submitted this frame.
        // Fired ~59 out of every 60 frames when the overlay rate-limits itself
        // to 60 FPS while the game runs at 400-1500 FPS.
        // Skipping avoids 14+ COM state-query calls (StateBackup::backup),
        // 3× Map/Unmap on DYNAMIC buffers, 14+ COM state-set calls (restore),
        // and a redundant CreateRenderTargetView — eliminating the primary source
        // of DX11 pipeline interference during high-FPS game rendering.
